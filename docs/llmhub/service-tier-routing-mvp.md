...
> 本文件早期标题沿用“服务档位”。当前新 Token 的实际选择项已经改为“模型族 + 倍率范围”，四个档位只保留为界面辅助标签；旧 Token 和旧分组请求仍按兼容规则运行。下文历史四档描述仅适用于未携带 `hub_routing_policy` 的旧路径。

## 目标

先跑通“渠道商供给 -> 倍率范围 Token -> 渠道商优先 -> 同倍率平台兜底 -> 扣费与日志追踪”的最小闭环，不在第一版引入动态并发、实时质量调权或自动来源识别。

## 已实现

- 新 Key 实际保存“模型族 + 倍率范围”策略；四个服务档位只作为按最高倍率显示的辅助标签，旧 `GroupRatio` 不参与新 Key 路由或计费。
- 管理员可在 `/system-settings/models/service-tiers-routing` 配置各模型族倍率边界、未知模型族开关和高品质渠道商白名单；倍率范围统一为 `0.01-100x`，步长 `0.001`，支持 `1x` 以上供给。
...
- 每个 `Channel + Model` 只按平台统一定价和渠道供给倍率参与候选匹配；服务档位标签由管理员配置的模型族边界计算，不改变实际倍率。
- 根域用户创建 API 密钥时可为多个模型族分别选择倍率范围；渠道商子域名用户只能选择该渠道商已发布的精确倍率。
- 渠道商子域名请求优先选择该渠道商在密钥精确倍率约束下的可用渠道，失败或无候选时进入平台公共池，但仍严格遵守同一倍率约束。
...
- 异步任务使用订阅额度时，最终补扣遇到订阅额度不足会复用同步请求的 `allow_wallet_overflow` 规则：允许时在同一结算事务中从钱包补差，禁止时保留待恢复状态；订阅记录、钱包、Token 和任务额度不会出现半成功。只有明确的订阅额度不足才允许钱包兜底，订阅不存在或数据库错误不会被误判为可兜底。
- 钱包最终补扣和订阅溢出到钱包都使用带余额条件的原子更新；有限额 Token 的同步结算、异步结算和恢复调账共用同一个原子扣减，不允许钱包或 Token 变成负数。同步钱包不足会拒绝补扣，异步结算或 Token 恢复不足会保留 `pending`；无限额 Token 只累计 `used_quota`，其展示用 `remain_quota` 不下降。
...
- 循环保护使用平台签名的 `X-LLM-Hub-Hop` 和并发请求指纹：标准 HTTP、multipart/form、任务提交、Midjourney 与 Realtime WebSocket 入口达到第 3 跳时固定返回 HTTP `400` 和 `request_loop_detected`；非空非 JSON `POST` 也进入原始请求体指纹保护。这里使用 `400` 是为了阻止旧版 New API 和通用客户端把 `508` 当作可重试的 5xx 错误放大循环；准确语义由错误码表达。普通请求不重试；服务档位请求只在既有重试预算内排除循环 Channel 后切换候选，循环失败不扣费、不结算，也不自动禁用 Channel。
...
- `service/hub_marketplace_flow_test.go` 覆盖渠道商和供给渠道创建、模型探测成功、手工上架后生成倍率 Ability、倍率策略 Token、子域渠道商优先、失败后公共池兜底、按最终供给倍率扣除用户与 Token 额度、只给最终渠道商结算收益，以及 `hub_attempts` 保留失败与成功两次尝试。
- 回归场景固定入口渠道商倍率为 `0.5x`、兜底渠道商倍率为 `0.6x`，验证最终按 `0.6x` 扣费，入口渠道商不获得该请求收益；另有 `5.5x/6x` 高倍率边界测试。
- 自动化用例使用确定性的本地数据库和模拟上游失败，不依赖真实网络；真实上游协议、超时和流式中断仍按下方验收流程进行人工验证。
- 当前自动化以模型/服务组合测试为主，尚未替代真实 Controller + `httptest.Server` 的黑盒链路；真实 Host、Token、重试、上游 usage 和最终账单仍以人工验收为准。
...
- 如果上游已经产生超出预扣的实际用量，但用户钱包或有限额 Token 无法覆盖最终补差，系统拒绝下溢并保留原预扣；消费日志、使用统计和渠道商收益都按已保留的预扣额度结算，未收取的差额不由平台垫付。是否允许欠费或限制最大生成量属于后续产品口径，本轮不新增债务账本。
- Midjourney 原任务续作/种子入口的倍率策略复查仍需单独补齐；管理端渠道测试日志的供给倍率口径已补齐，但两者仍不在当前文本、Responses、Realtime、音频和通用异步任务主链路内。
