...
func quotaNotificationContent(notifyType string, prompt string, remaining string, topUpLink string) (string, []interface{}) {
	if notifyType == dto.NotifyTypeWebhook {
		return "{{value}}，当前剩余额度为 {{value}}，为了不影响您的使用，请及时充值。\n充值链接：{{value}}",
			[]interface{}{prompt, remaining, topUpLink}
	}
	return "{{value}}，当前剩余额度为 {{value}}，为了不影响您的使用，请及时充值。<br/>充值链接：<a href='{{value}}'>{{value}}</a>",
		[]interface{}{prompt, remaining, topUpLink, topUpLink}
}
...
func PreWssConsumeQuota(ctx *gin.Context, relayInfo *relaycommon.RelayInfo, usage *dto.RealtimeUsage) error {
	if relayInfo == nil || usage == nil || relayInfo.PriceData.UsePrice {
		return nil
	}
	...
	if tieredOK, tieredQuota, _ := TryTieredSettle(relayInfo, billingexpr.TokenParams{
	...
	logger.LogInfo(ctx, "realtime streaming quota reserved, target quota: "+fmt.Sprintf("%d", quota))
	return nil
}
...
func PostWssConsumeQuota(ctx *gin.Context, relayInfo *relaycommon.RelayInfo, modelName string,
	...
	billingUsage := realtimeUsageForBilling(usage)
	...
	if hubAttemptResult == HubAttemptResultFailed {
		...
		logger.LogError(ctx, fmt.Sprintf("abnormal realtime stream ended without usable output, userId %d, channelId %d, tokenId %d, model %s, pre-consumed quota %d", relayInfo.UserId, relayInfo.ChannelId, relayInfo.TokenId, modelName, relayInfo.FinalPreConsumedQuota))
...
func realtimeUsageForBilling(usage *dto.RealtimeUsage) *dto.Usage {
	if usage == nil {
		return nil
	}
	return &dto.Usage{
	...
	gopool.Go(func() {
		...
		if quotaTooLow {
			...
			if notifyType == dto.NotifyTypeBark {
				...
				content, values = quotaNotificationContent(notifyType, prompt, logger.FormatQuota(relayInfo.UserQuota), topUpLink)
			}
...
func checkAndSendSubscriptionQuotaNotify(relayInfo *relaycommon.RelayInfo) {
	gopool.Go(func() {
		...
		if notifyType == dto.NotifyTypeBark {
			...
			content, values = quotaNotificationContent(notifyType, prompt, logger.FormatQuota(int(remaining)), topUpLink)
		}
