...
var errWalletQuotaInsufficient = errors.New("wallet quota is insufficient")
...
func (s *BillingSession) Settle(actualQuota int) error {
	...
	if funding, ok := s.funding.(*SubscriptionFunding); ok {
		s.relayInfo.SubscriptionPostDelta += funding.settledSubscriptionDelta
		s.relayInfo.WalletOverflowPostDelta += funding.settledWalletDelta
	}
	s.settled = true
	return nil
}
	...
	if err := s.funding.PreConsume(effectiveQuota); err != nil {
		...
		if errors.Is(err, errWalletQuotaInsufficient) {
			return types.NewErrorWithStatusCode(
				fmt.Errorf("用户额度不足，无法预扣 %s", logger.FormatQuota(effectiveQuota)),
...
func (s *BillingSession) reserveFunding(delta int) error {
	switch funding := s.funding.(type) {
	case *WalletFunding:
		reserved, err := model.TryDecreaseUserQuota(funding.userId, delta)
		if err != nil {
			return types.NewError(err, types.ErrorCodeUpdateDataError, types.ErrOptionWithSkipRetry())
		}
		if !reserved {
			return types.NewErrorWithStatusCode(
				fmt.Errorf("用户额度不足，无法为更高价格的重试预留 %s", logger.FormatQuota(delta)),
				types.ErrorCodeInsufficientUserQuota,
				http.StatusForbidden,
				types.ErrOptionWithSkipRetry(),
				types.ErrOptionWithNoRecordErrorLog(),
