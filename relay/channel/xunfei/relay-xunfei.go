...
func xunfeiStreamHandler(c *gin.Context, info *relaycommon.RelayInfo, textRequest dto.GeneralOpenAIRequest, appId string, apiSecret string, apiKey string) (*dto.Usage, *types.NewAPIError) {
	...
	dataChan, stopChan, err := xunfeiMakeRequest(textRequest, domain, authUrl, appId, info.IsHubSupplyChannel)
	if err != nil {
...
func xunfeiMakeRequest(textRequest dto.GeneralOpenAIRequest, domain, authUrl, appId string, isHubSupplyChannel bool) (chan XunfeiChatResponse, chan bool, error) {
	d := websocket.Dialer{
		HandshakeTimeout: 5 * time.Second,
	}
	if isHubSupplyChannel {
		d.NetDialContext = service.GetPublicNetworkDialContext()
	}
