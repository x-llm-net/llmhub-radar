...
func TestRelayInfoObserveMeaningfulStreamData(t *testing.T) {
	tests := []struct {
		...
		{name: "openai tool name", data: `{"choices":[{"delta":{"tool_calls":[{"function":{"name":"lookup","arguments":""}}]}}]}`, want: true},
		{name: "openai tool arguments", data: `{"choices":[{"delta":{"tool_calls":[{"function":{"arguments":"{\"q\":"}}]}}]}`, want: true},
		{name: "responses text", data: `{"type":"response.output_text.delta","delta":"hello"}`, want: true},
		{name: "responses function arguments", data: `{"type":"response.function_call_arguments.delta","delta":"{\"q\":"}`, want: true},
		{name: "responses custom tool input", data: `{"type":"response.custom_tool_call_input.delta","delta":"query"}`, want: true},
		{name: "responses empty function arguments", data: `{"type":"response.function_call_arguments.delta","delta":""}`},
		{name: "responses lifecycle", data: `{"type":"response.created","response":{"status":"in_progress"}}`},
		{name: "claude text", data: `{"type":"content_block_delta","delta":{"type":"text_delta","text":"hello"}}`, want: true},
		{name: "claude tool input", data: `{"type":"content_block_delta","delta":{"type":"input_json_delta","partial_json":"{\"q\":"}}`, want: true},
		{name: "gemini text", data: `{"candidates":[{"content":{"parts":[{"text":"hello"}]}}]}`, want: true},
		{name: "gemini function call", data: `{"candidates":[{"content":{"parts":[{"functionCall":{"name":"lookup","args":{"q":"x"}}}]}}]}`, want: true},
