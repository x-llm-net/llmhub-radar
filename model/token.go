...
type Token struct {
	...
	HubRoutingPolicy   string         `json:"-" gorm:"type:text"`
...
func (token *Token) Update() (err error) {
	err = DB.Model(token).Select("name", "status", "expired_time", "remain_quota", "unlimited_quota",
		"model_limits_enabled", "model_limits", "allow_ips", "group", "cross_group_retry", "auto_groups", "hub_routing_policy").Updates(token).Error
	if shouldUpdateRedis(true, err) {
