...
type User struct {
	...
	// Registration source is an immutable attribution record for analytics.
	// It does not affect authorization, routing, billing, or invitation logic.
	RegistrationTenantId     int                        `json:"registration_tenant_id,omitempty" gorm:"column:registration_tenant_id;index"`
	RegistrationProviderId   int                        `json:"registration_provider_id,omitempty" gorm:"column:registration_provider_id;index"`
	RegistrationSource       string                     `json:"registration_source,omitempty" gorm:"column:registration_source;type:varchar(32);index"`
	RegistrationTenantName   string                     `json:"registration_tenant_name,omitempty" gorm:"-"`
	RegistrationProviderName string                     `json:"registration_provider_name,omitempty" gorm:"-"`
...
func GetAllUsers(pageInfo *common.PageInfo, sortOptions ...UserSortOptions) (users []*User, total int64, err error) {
	...
	if enrichErr := EnrichUserRegistrationSources(users); enrichErr != nil {
		common.SysError("failed to enrich user registration sources: " + enrichErr.Error())
	}
...
func (user *User) UpdateWithTx(tx *gorm.DB, updatePassword bool) error {
	...
	if err = tx.Model(&current).Omit(
		"quota", "used_quota", "request_count", "auth_version",
		"registration_tenant_id", "registration_provider_id", "registration_source",
...
// TryDecreaseUserQuota atomically reserves quota only when the current
// database balance covers it. Reservation paths must not use the batched
// updater because a later retry needs an immediately authoritative result.
func TryDecreaseUserQuota(id int, quota int) (bool, error) {
	if quota < 0 {
