...
The script reads the active provider URL and credential from Codex, then makes one request to `<base_url>/images/generations` with `gpt-image-2` and requests PNG output. It does not call `/responses`, retry a failed request, switch providers, or infer image capability from the current conversational model. The selected LLM-Hub key must include `gpt-image-2` and have a healthy Images API route.
