...
docker exec llm-hub-mysql sh -c 'exec mysqldump --no-tablespaces --single-transaction --routines --triggers -u"$MYSQL_USER" -p"$MYSQL_PASSWORD" --databases "$MYSQL_DATABASE"' > "$backup_dir/mysql.sql"
